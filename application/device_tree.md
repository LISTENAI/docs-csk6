...
```
开发者需要重点关注`.dts`和`.overlay`文件：

- `.dts` 文件

一般是在zephyr SDK内部，非SDK开发者一般是不需要关心内部的设备树设计的，如果您想了解CSK6的设备定义，可以查看sdk内部的设备树相关文件，或者您需要根据硬件适配一个新的板型，可以在项目中自定义 *dts* 来实现，具体实现步骤可以查阅[自定义板型](board.md)章节。

- `.overlay`文件

是项目级别的设备树配置文件，除了csk6 sdk支持的板型以外，开发者可通过`.overlay`来增加更多的设备树配置。
...
.dts在csk6 sdk中的路径，以`csk6002_9s_nano`板型为例：
```c
...
#### 设备树绑定文件(.yaml文件)
bindings文件在zephyr工程中是以.yaml文件形式存在，bindings顾名思义，绑定的意思，他作为c语言和设备树之前的桥梁存在。bingdings可以解释设备树中设备节点的属性代表什么意思。前面提到的将设备树转换为c语言头文件，就是根据bindings文件中定义的规则来转换的。
...
例如`csk6 sdk/boards/arm/csk6002_9s_nano/csk6002_9s_nano.dts`中lable为`green_pwm_led`的设备树配置如下：
...
对应的bindings文件目录：`csk6 sdk/dts/bindings/led/pwm-leds.yaml`。
`pwm-leds.yaml`内容：
```c
...
```
构建系统会根据`dts`文件中`compatible` 匹配对应的bindings文件，从而完成设备树的解析，例如`csk6002_9s_nano.dts`中的pwmleds段配置会根据`compatible = "pwm-leds"`匹配对应的bindings文件`dts/bindings/led/pwm-leds.yaml.yaml`。
### 设备树语法
