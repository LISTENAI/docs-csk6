...
初始化一个k_poll_event实例，使用轮询功能需要包含头文件`#include <zephyr/kernel.h>`。轮询事件到来，会放置在事件数组中，后面传递给`k_poll`接口。
...
| ----- | ------------------------------------------- |
| event | 初始化的事件                                |
| type  | 事件类型，定义的值为K_POLL_TYPE_xxx类型的宏 |
| mode  | 固定用K_POLL_MODE_NOTIFY_ONLY               |
| obj   | 内核对象或轮询信号                          |
