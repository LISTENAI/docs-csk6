...
&spi0 {
        ...
        reg = <0>;
        cmd-data-gpios = <&gpioa 8 1>; 
        reset-gpios = <&gpioa 9 1>; 
...
void main(void)
{
	...
	if (IS_ENABLED(CONFIG_LV_Z_POINTER_KSCAN)) {
        ...
        lv_obj_t *top_button;
        top_button = lv_btn_create(lv_scr_act());
		lv_obj_align(top_button, LV_ALIGN_TOP_MID, 0, 0);
		top_label = lv_label_create(top_button);

        lv_obj_t *bottom_button;
        bottom_button = lv_btn_create(lv_scr_act());
		lv_obj_align(bottom_button, LV_ALIGN_BOTTOM_MID, 0, 0);
		bottom_label = lv_label_create(bottom_button);
	} else {
